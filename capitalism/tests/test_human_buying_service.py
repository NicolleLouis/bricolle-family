...
def test_buying_service_purchases_affordable_objects():
    ...
    assert buyer.get_object_quantity(ObjectType.BREAD) == 2
    assert seller_one.get_object_quantity(ObjectType.BREAD) == 0
    assert seller_two.get_object_quantity(ObjectType.BREAD) == 0
    assert buyer.owned_objects.filter(id=object_one.id).exists() is False
    assert buyer.owned_objects.filter(id=object_two.id).exists() is False
...
class _FixedPriceValuation:
    ...
    def estimate_price(self, _human, object_type: str, owned_quantity_override=None) -> float:
...
def test_buying_service_recalculates_price_between_purchases():
    ...
    assert pricey_seller.get_object_quantity(ObjectType.BREAD) == 1
